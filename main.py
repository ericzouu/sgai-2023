...
class Main(object):
    ...
    def __init__(self, is_automode, is_disable):
        self.data_fp = os.getenv("SGAI_DATA", default=os.path.join('data', 'test_images'))
        ...
        if not is_automode:  # Launch UI gameplay
            is_poor=random.choice([True, False])
            ...
            print('suggestions do not work atm')
            simon = MachineInterface(None, None, None, is_automode)
            self.scorekeeper.is_auto=args.automode
